...
- (void)alipay {
    ...
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    NSString *urlStr = [NSString stringWithFormat:@"%@?channelType=%@&money=%@&routeId=%@",URL_Recharge,_alipayRoute.channelType,self.needMoneyLab.text,_alipayRoute.routeId];
    [[NetworkManager sharedManager] postJSON:urlStr parameters:dic completion:^(id responseData, RequestState status, NSError *error) {
        if (status==Request_Success) {
...
- (void)wechatPay {
    if (![WXApi isWXAppInstalled] && ![WXApi isWXAppSupportApi]) {
        ...
        NSMutableDictionary *dic = [NSMutableDictionary dictionary];
        NSString *urlStr = [NSString stringWithFormat:@"%@?channelType=%@&money=%@&routeId=%@",URL_Recharge,_wechatRoute.channelType,self.needMoneyLab.text,_wechatRoute.routeId];
        [[NetworkManager sharedManager] postJSON:urlStr parameters:dic completion:^(id responseData, RequestState status, NSError *error) {
            if (status==Request_Success) {
